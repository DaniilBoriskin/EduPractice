...
function clickLog(event) {
	document.getElementById('enter-box').hidden = false;
	if (event.target.id === 'log-in' && user.name === '') {
		enterTitle.textContent = 'Authorization';
	}
	if(event.target.id === 'sign-up' && user.name === ''){
		enterTitle.textContent = 'Registration';
	}
}

function hideEnterForm(event){
	console.log(event.keyCode);
	if(event.keyCode === 27){
		console.log("dsa");
		document.getElementById('enter-box').hidden = true;
	}
}

postContainer.addEventListener('click', clickLike);
document.querySelector('header').addEventListener('click', clickLog);
document.getElementById('enter-form').addEventListener('keydown', hideEnterForm);




addPost(new PhotoPost('1', '1', new Date(2009, 6, 10), 'Алиса', 'Resourses/img/picture2_skolko-jagod-sobe_342560_p0.jpg', ['#cool', '#r'], ['Алиса']));
addPost(new PhotoPost('2', 'qwer', new Date(2007, 6, 10), 'Олег', 'Resourses/img/picture2_skolko-jagod-sobe_342560_p0.jpg', ['#cool', '#wow'], ['Олег', 'Алиса']));
addPost(new PhotoPost('3', '1', new Date(2001, 6, 10), 'Алиса', 'Resourses/img/picture2_skolko-jagod-sobe_342560_p0.jpg', ['#cool', '#s'], ['Алиса']));
addPost(new PhotoPost('4', '1', new Date(2008, 5, 10), 'Богдан', 'Resourses/img/picture2_skolko-jagod-sobe_342560_p0.jpg', ['#q', '#r'], ['Алиса']));
consoleAuthors();
removePost(1);
guestView();
editPost('3', {
